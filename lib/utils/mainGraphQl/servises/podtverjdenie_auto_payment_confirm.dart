...
Future autoPaymentConfirm(
    {String? smsCode, String? claimsId, String? tokenFromLocaleStorage}) async {
  ...
  QueryResult queryResult = await qlClient.mutate(
    ...
    MutationOptions(
        ...
        document: gql(
          gql_string.autoPayConfirm,
        ),
        variables: {
          "smsCode": smsCode == null ? null : int.parse(smsCode),
          "claimsId": claimsId == null ? null : int.parse(claimsId)
        }),
  );
  return queryResult.data?['autoPaymentConfirm']['msg'];
}
